/*
  # Add Public Token-Based Access for Assessments

  1. Changes
    - Add anonymous access policies for assessment instances via token
    - Allow public access to questions and options for token-based assessments
    - Enable anonymous users to submit responses via valid tokens
    - Maintain security by only allowing access through valid, non-expired tokens

  2. Security
    - Token validation ensures only valid assessment instances are accessible
    - Expiration checking prevents access to expired assessments
    - No organization data is exposed to anonymous users
    - Responses are linked to the specific instance via token validation

  3. Important Notes
    - Anonymous access is strictly limited to token-based assessment taking
    - Admin functions remain protected and require authentication
